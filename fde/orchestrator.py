...
PROVIDERS: dict[str, dict[str, Any]] = {
    "codex": _provider(
        ...
        description="ChatGPT-authenticated Codex CLI for evidence synthesis, narration, and visual direction.",
    ...
    "anthropic_api": _provider(
        ...
        models_by_capability={
            "structured": ["claude-fable-5", "claude-opus-4-8", "claude-sonnet-5", "claude-haiku-4-5"]
        },
        description="Anthropic Messages API with contract-constrained structured output.",
    ...
    "grok_cli": _provider(
        ...
        description="Subscription-backed Grok CLI for structured tasks and built-in Imagine media tools.",
        ...
        integration_notes=(
            "The requested Imagine model, quality, resolution, aspect ratio, and duration are written into the "
            "Grok CLI tool brief and saved with the returned media. The CLI may internally route its Imagine tool."
        ),
    ),
    "kimi_api": _provider(
        "kimi_api",
        ...
        capabilities=["structured"],
        models_by_capability={
            "structured": ["kimi-k3", "kimi-k2.7-code", "kimi-k2.7-code-highspeed", "kimi-k2.6", "kimi-k2.5"]
        },
    ...
    "gemini_api": _provider(
        ...
        description="Gemini API for structured reasoning, current image generation, and current video generation.",
        ...
        integration_notes="Veo 3.1 requests at 1080p or 4k are normalized to an eight-second provider generation before local trimming.",
    ),
    ...
    "openai_image_api": _provider(
        ...
        integration_notes="OpenAI exposes native pixel dimensions rather than generic 1K/2K labels.",
    ),
    "google_tts": _provider(
        ...
        capabilities=["audio"],
        models_by_capability={
            "audio": ["gemini-3.1-flash-tts-preview", "gemini-2.5-flash-preview-tts", "gemini-2.5-pro-preview-tts"]
        },
    ...
    "manual_upload": _provider(
        ...
        models_by_capability={
            "image": ["External image tool"],
            "video": ["External video tool"],
            "audio": ["External narration"],
        },
    ...
    "mock": _provider(
        ...
        models_by_capability={
            capability: ["Deterministic Demo"]
            for capability in ["structured", "image", "video", "audio", "alignment", "render"]
        },
        description="Deterministic offline provider for pipeline and Studio tests.",
        docs_url="https://github.com/ananthcjayan12/failure-documentary-engine",
    ),
}
...
TASK_DEFINITIONS: list[dict[str, Any]] = [
    ...
    {
        "id": "shot_planner",
        "label": "Audio-led visual director",
        "stage": 4,
        "pipeline_stage": "shots",
        "group": "Shots",
        "capability": "structured",
        "description": "Design visual treatments around deterministic word and pause boundaries.",
    },
    {
        "id": "image_generator",
    ...
    },
    {
        ...
        "label": "Animatic renderer",
        "stage": 6,
        "pipeline_stage": "animatic",
        "group": "Image + sound preview",
        "capability": "render",
        "description": "Combine approved images, exact narration, and restrained ambience.",
    },
    {
        "id": "video_generator",
        ...
        "group": "Videos",
        "capability": "video",
        "description": "Generate selected clips with explicit model, resolution, duration, and aspect ratio.",
    },
    {
        "id": "final_renderer",
        "label": "Final preview renderer",
        "stage": 8,
        "pipeline_stage": "final_preview",
        "group": "Final preview",
        "capability": "render",
        "description": "Assemble approved videos, image fallbacks, and master narration.",
    },
]
TASK_BY_ID = {item["id"]: item for item in TASK_DEFINITIONS}

# Old task IDs are accepted only to migrate an already-saved V1 configuration or a compatibility call.
TASK_ALIASES = {
...
PROMPT_PACKS: dict[str, dict[str, Any]] = {
    "aviation_investigation": {
        "id": "aviation_investigation",
        "label": "Aviation Investigation",
        "description": "Technically credible air-crash and disappearance investigations.",
        "instructions": (
            "Use restrained aviation-investigation language. Distinguish recorded evidence, official findings, "
            "expert inference, and unresolved theory. Preserve aircraft, cockpit, geography, and timeline continuity."
        ),
    },
    "engineering_failure": {
        "id": "engineering_failure",
        "label": "Engineering Failure",
        "description": "Bridge, building, industrial, and infrastructure failures.",
        "instructions": (
            "Explain the failure chain through load, material, process, and organizational decisions. "
            "Prefer evidence over spectacle and separate the initiating event from contributing conditions."
        ),
    },
    "cyber_incident": {
        "id": "cyber_incident",
        ...
        "instructions": (
            "Make digital systems understandable without inventing interfaces. Separate confirmed indicators, "
            "attributed behavior, and speculation."
        ),
    },
    "space_mission": {
        "id": "space_mission",
        "label": "Space Mission Failure",
        "description": "Launch, spacecraft, and mission investigations.",
        "instructions": (
            "Maintain mission chronology, vehicle configuration, and orbital realism. Explain tradeoffs clearly "
            "and avoid fictional telemetry."
        ),
    },
    "general_documentary": {
        "id": "general_documentary",
        "label": "General Investigation",
        "description": "Neutral default for any major failure or mystery.",
        "instructions": (
            "Build curiosity through evidence and unanswered questions, not exaggeration. Clearly label uncertainty "
...
PROFILES: dict[str, dict[str, Any]] = {
    "highest_quality": {
        ...
        "description": "Claude and GPT-5.6 for the spoken story, Gemini Pro visual direction, 2K Grok images, and 720p Grok video.",
        "routes": _profile_routes(),
    },
    "balanced": {
        "id": "balanced",
        "label": "Balanced",
        "description": "GPT-5.6 Terra and Gemini Flash for structured work, standard 1K images, and 720p video.",
        "routes": _profile_routes({
    ...
    "api_first": {
        "id": "api_first",
        "label": "API First",
        "description": "Claude, Kimi, and Gemini APIs with Gemini image and video generation.",
        "routes": _profile_routes({
    ...
    "subscription_cli": {
        ...
        "description": "Codex, Gemini CLI, and Grok CLI minimize structured API usage.",
        "routes": _profile_routes({
            "research": ("codex", "gpt-5.6-terra", "medium"),
    ...
    "fast_low_cost": {
        "id": "fast_low_cost",
        "label": "Fast / Low Cost",
        "description": "Kimi K2.7 high-speed, Gemini Flash-Lite, 1K images, and 480p video.",
        "routes": _profile_routes({
    ...
    "offline": {
        "id": "offline",
        "label": "Offline Test",
        "description": "No external calls; validates restarts, routing, media, and rendering.",
        "routes": {
...
PROFILES["api_first"]["routes"]["image_generator"].update({
    "resolution": "2K", "quality": "high", "aspect_ratio": "16:9"
})
PROFILES["api_first"]["routes"]["video_generator"].update({
...
DEFAULT_RULES: list[dict[str, Any]] = [
    {
        "id": "long_narration_quality",
        "label": "Long documentary narration",
        "enabled": True,
        "task_id": "narration_writer",
...
def _validate_media_settings(provider: dict[str, Any], capability: str, route_config: dict[str, Any]) -> None:
    ...
    ratio = str(route_config.get("aspect_ratio", ""))
...
def _validate_route(task_id: str, route_config: dict[str, Any]) -> None:
    definition = TASK_BY_ID[task_id]
    capability = definition["capability"]
    provider_id = str(route_config.get("provider", ""))
    ...
    model = str(route_config.get("model", ""))
    if models and model not in models and provider_id != "custom_cli":
        raise ValueError(f"Model {model} is not a verified {capability} model for {provider['label']}")
    _validate_media_settings(provider, capability, route_config)

    fallback_id = str(route_config.get("fallback_provider", ""))
    if not fallback_id:
        return
    if fallback_id not in PROVIDERS:
        raise ValueError(f"Unknown fallback provider: {fallback_id}")
    fallback = PROVIDERS[fallback_id]
    if capability not in fallback.get("capabilities", []):
    ...
    fallback_model = str(route_config.get("fallback_model", ""))
...
def merge_orchestrator_config(saved: dict[str, Any] | None) -> dict[str, Any]:
    ...
    saved = saved or {}
    saved_version = int(saved.get("version") or 0)

    # V0.5/V0.6 workspaces used a different visual-first task graph and provider catalogue.
    # Their route IDs are not silently carried into V1 because that produced invalid combinations
    # such as Claude Code for the API-only narration route or authenticated-default as an image model.
    # Provider command overrides and prompt-pack customizations are still preserved.
    if saved_version >= CONFIG_VERSION:
        for raw_task_id, values in (
            saved.get("tasks", {}) if isinstance(saved.get("tasks"), dict) else {}
        ).items():
            task_id = TASK_ALIASES.get(raw_task_id, raw_task_id)
            if task_id not in config["tasks"] or not isinstance(values, dict):
                continue
            ...
            try:
                ...
                continue
            config["tasks"][task_id] = candidate
        active_profile = str(saved.get("active_profile") or "custom")
        config["active_profile"] = active_profile if active_profile in PROFILES or active_profile == "custom" else "custom"
    else:
        config["active_profile"] = "highest_quality"

    prompt_pack = str(saved.get("active_prompt_pack") or config["active_prompt_pack"])
    if prompt_pack in PROMPT_PACKS:
        config["active_prompt_pack"] = prompt_pack

    for provider_id, values in (
        saved.get("providers", {}) if isinstance(saved.get("providers"), dict) else {}
    ).items():
        if provider_id in config["providers"] and isinstance(values, dict):
            config["providers"][provider_id].update(values)

    for pack_id, values in (
        saved.get("prompt_packs", {}) if isinstance(saved.get("prompt_packs"), dict) else {}
    ).items():
        if pack_id in config["prompt_packs"] and isinstance(values, dict):
            config["prompt_packs"][pack_id].update(values)

    if saved_version >= CONFIG_VERSION and isinstance(saved.get("rules"), list):
        migrated_rules = []
        for item in saved["rules"]:
            if not isinstance(item, dict):
                continue
            migrated = copy.deepcopy(item)
            migrated["task_id"] = TASK_ALIASES.get(migrated.get("task_id"), migrated.get("task_id"))
            if migrated.get("task_id") in TASK_BY_ID:
                migrated_rules.append(migrated)
        config["rules"] = migrated_rules
...
def provider_health(provider_id: str, provider_overrides: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if provider_id == "custom_cli":
        configured = bool(
            str(overrides.get("command_template") or "").strip()
            or str(overrides.get("media_command_template") or "").strip()
        )
        return {
            "provider_id": provider_id,
            "status": "ready" if configured else "unconfigured",
            "healthy": configured,
            "detail": "Custom command configured" if configured else "Add a command template",
        }
    if provider_id == "hyperframes":
        root = Path(__file__).resolve().parents[1]
        found = bool(
            shutil.which("hyperframes")
            or (root / "node_modules/.bin/hyperframes").exists()
            or shutil.which("npx")
        )
        return {
            "provider_id": provider_id,
            "status": "ready" if found else "missing",
            "healthy": found,
            "detail": "HyperFrames available" if found else "Run npm install",
        }
...
def resolve_task(config: dict[str, Any], task_id: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for rule in config.get("rules", []):
        if (
            ...
            or TASK_ALIASES.get(rule.get("task_id"), rule.get("task_id")) != task_id
        ):
            continue
    ...
    _validate_route(task_id, resolved)
    ...
    resolved.update({
        ...
        "provider_docs_url": provider["docs_url"],
    ...
    fallback_id = str(resolved.get("fallback_provider") or "")
    ...
    resolved.update({
        ...
        "prompt_pack_label": pack.get("label", pack_id),
...
def public_payload(config: dict[str, Any]) -> dict[str, Any]:
    health = {
        provider_id: provider_health(provider_id, config.get("providers", {}).get(provider_id))
    ...
    for definition in TASK_DEFINITIONS:
        ...
        item.update({
            ...
            "health": health.get(item.get("provider"), {}),
        })
        tasks.append(item)
    return {
        **copy.deepcopy(config),
        "tasks": tasks,
        "provider_catalog": [
            copy.deepcopy(value) | {"health": health[key]}
            for key, value in PROVIDERS.items()
        ],
        "profiles": [
            copy.deepcopy(value) | {"task_count": len(value["routes"])}

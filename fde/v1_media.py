...
def prepare_media_jobs(project_dir: Path, media_type: str) -> V1MediaManifest:
    ...
    for shot in shots.shots:
        ...
        if old and old.prompt == prompt and old.duration_seconds == (0 if media_type == "image" else shot.duration):
            jobs.append(old)
            continue
        jobs.append(
            V1MediaJob(
                job_id=f"{media_type.upper()}_{shot.shot_id}",
                ...
                output=output,
                reference=reference,
                duration_seconds=0 if media_type == "image" else shot.duration,
                resolution="720p",
                aspect_ratio="16:9",
            )
        )
...
def generate_media_jobs(
    ...
    *,
    ...
    project_dir = Path(project_dir)
    manifest = prepare_media_jobs(project_dir, media_type)
        ...
        try:
            ...
            if provider == "mock" and media_type == "image":
                ...
                record = generate_one(
                    ...
                    prompt=job.prompt + ("\nRequired output resolution: 720p." if media_type == "video" else ""),
                    destination=output,
                    media_type=media_type,
                    cwd=project_dir,
            ...
            job.error = None
            job.updated_at = utc_now()
            job_dir = output.parent
            write_json(job_dir / "generation.json", {
                **record,
                "shot_id": job.shot_id,
                "media_type": media_type,
            ...
            if str(route.get("provider")) in {"manual_upload", "chatgpt_ui", "grok_ui"}:
    ...
    for job in manifest.jobs:
        ...
        if not output.exists():
            if media_type == "video" and _image_path(project_dir, job.shot_id).exists():
                # V1 intentionally allows a reviewed shot to remain a still image.
                job.status = "rejected"
...
def _voiceover_path(project_dir: Path) -> tuple[Path, AudioManifest]:
    ...
    preferred = project_dir / manifest.voiceover_wav
    return preferred, manifest
...
def render_animatic(project_dir: Path, *, width: int = 1280, height: int = 720, fps: int = 24) -> AnimaticManifest:
    ...
    for index, shot in enumerate(shots.shots, start=1):
        ...
        sound_plan.append({
            ...
            "start": shot.start,
            "end": shot.end,
            "hint": shot.sound_hint,
            "file": str(cue.relative_to(project_dir)) if cue.exists() else None,
        })
    ...
    # Low-level pink ambience provides the requested sound context while remaining safely under narration.
    _run([

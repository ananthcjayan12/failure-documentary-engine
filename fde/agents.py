...
class ManualAgent(StructuredAgent):
    ...
    def run(self, *, stage: str, prompt: str, output_model: type[BaseModel], request_dir: Path) -> BaseModel:
        ...
        raise AgentPending(
            ...
            f"Then rerun with --consume-response."
        )
...
class CommandAgent(StructuredAgent):
    ...
    def run(self, *, stage: str, prompt: str, output_model: type[BaseModel], request_dir: Path) -> BaseModel:
        ...
        if pack_instructions:
            routed_prompt = (
                f"# Active prompt pack: {pack_id or 'workspace default'}\n\n"
                f"{pack_instructions}\n\n---\n\n{prompt}"
            )
        ...
        for attempt in range(retries + 1):
            attempts.append((self.command_template, model, f"primary attempt {attempt + 1}"))
        ...
        for template, selected_model, label in attempts:
            ...
            command = template.format(
                ...
                reasoning=shlex.quote(reasoning),
                temperature=shlex.quote(str(temperature)),
...
class RoutedAgent(StructuredAgent):
    """Provider-agnostic structured task executor used by Studio routes.

    The primary and fallback adapters are resolved before the subprocess starts,
    so one task may mix any compatible command CLI, Grok CLI, mock generator, or
    manual bridge without changing the pipeline implementation.
    """
    ...
    def _execute(
        ...
        if adapter in {"command", "custom_cli"}:
            ...
            return self._command_once(
                ...
                output_path=output_path, model=item.get("model", ""), timeout=timeout,
            )
        if adapter == "mock":
            value = MockAgent(self.context).run(
    ...
    def _command_once(
        *, template: str, stage: str, prompt_path: Path, output_path: Path,
        ...
        command = template.format(
            ...
            reasoning=shlex.quote(os.environ.get("FDE_LLM_REASONING", "medium")),
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"command failed ({completed.returncode}): {(completed.stderr or completed.stdout)[-4000:]}"
            )
        ...
        if output_model is ResearchDossier:
            return ResearchDossier(
                ...
                claims=[
                    Claim(
                        claim_id="CLM_001",
                        statement="A major anomaly occurred.",
                        status="confirmed",
                        source_ids=["SRC_001"],
                        allowed_language="The evidence records a major anomaly.",
                    )
                ],
            )
            ...
            for chapter in chapters:
                for local in range(1, 4):
                    text = (
                        ...
                        f"It explains the evidence carefully, separates confirmed facts from theory, "
                        f"and prepares the next investigative reveal."
                    )
                ...
                for part in range(2):
                    ...
                    shots.append(
                        Shot(
                            ...
                            narration_ids=[segment.narration_id],
                            start=segment.estimated_start + part * half, duration=half,
                            visual_purpose=f"Support {segment.narration_id} with a distinct visual beat.",
                            visual_type=vtype,
                            suggested_visual=f"Reusable {vtype.replace('_', ' ')} scene for {segment.chapter_id}",
